...
char **argv;
{
  ...
  if(debug_algorithm)
    printf("Algorithm 'lqf()' called by switch %d\n", aSwitch->switchNumber);
	
  switch(action) {
  case SCHEDULING_USAGE:
    fprintf(stderr, "No Options for \"lqf\" scheduling algorithm:\n");
    break;
  ...
  if(debug_algorithm)
    printf("Algorithm 'lqf()' completed for switch %d\n", aSwitch->switchNumber);
}

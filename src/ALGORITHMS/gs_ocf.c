...
void
gs_ocf(action, aSwitch, argc, argv)
  SwitchAction action;
Switch *aSwitch;
int argc;
char **argv;
{
  ...
  if(debug_algorithm)
    printf("Algorithm 'gs_ocf()' called by switch %d\n", aSwitch->switchNumber);
	
  switch(action) {
  case SCHEDULING_USAGE:
    fprintf(stderr, "No Options for \"gs_ocf\" scheduling algorithm:\n");
    break;
  ...
  if(debug_algorithm)
    printf("Algorithm 'gs_ocf()' completed for switch %d\n", aSwitch->switchNumber);
}

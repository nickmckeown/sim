...
void
lpf_delay(action, aSwitch, argc, argv)
SwitchAction action;
Switch *aSwitch;
int argc;
char **argv;
{
	...
	if(debug_algorithm)
		printf("Algorithm 'lpf_delay()' called by switch %d\n", aSwitch->switchNumber);
	
	switch(action) {
		case SCHEDULING_USAGE:
			fprintf(stderr, "No Options for \"lpf_delay\" scheduling algorithm:\n");
			break;
	...
	if(debug_algorithm)
		printf("Algorithm 'lpf_delay()' completed for switch %d\n", aSwitch->switchNumber);
}
...
Switch *aSwitch;
{
   ...
   if( !lqf_match )
	...
	/*	find_lqf_match(lqf_match,n); */
	/* cal. col. and row sums */
	for(input=0; input<n; input++){
		inputBuffer = aSwitch->inputBuffer[input];	  
		for(output=0; output<aSwitch->numOutputs; output++){
		  col_sum[0][output] += inputBuffer->fifo[output]->number;
		  row_sum[0][input] += inputBuffer->fifo[output]->number;
		}
	}
	...
	/*
	for(input=0; input<n; input++){
	  col_sum[1][input] = col_sum[0][input];
	  row_sum[1][input] = row_sum[0][input];
	}
	*/

...
char **argv;
{
  ...
  if(debug_algorithm)
    printf("Algorithm 'iocf()' called by switch %d\n", aSwitch->switchNumber);
	
  switch(action) {
  case SCHEDULING_USAGE:
    fprintf(stderr, "Options for \"iocf\" scheduling algorithm:\n");
  ...
  case SCHEDULING_INIT:	
	...
	while( (c = getopt(argc, argv, "n:")) != EOF)
	  switch (c)
	    ...
	    case '?':
	      fprintf(stderr, "-------------------------------\n");
	      fprintf(stderr, "iocf: Unrecognized option -%c\n", optopt);
	      iocf(SCHEDULING_USAGE);
	      fprintf(stderr, "--------------------------------\n");
	      exit(1);
	    default:
	      break;
  ...
  if(debug_algorithm)
    printf("Algorithm 'iocf()' completed for switch %d\n", aSwitch->switchNumber);
}
...
int output;
{
  ...
  /**************************************/
	...
	/* !!!TEMPORARY:!!! If two or more the same, pick last in list */

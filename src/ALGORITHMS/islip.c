...
void
islip(action, aSwitch, argc, argv)
  SwitchAction action;
Switch *aSwitch;
int argc;
char **argv;
{
  ...
  if(debug_algorithm)
    printf("Algorithm 'islip()' called by switch %d\n", aSwitch->switchNumber);
	
  switch(action) {
  case SCHEDULING_USAGE:
    fprintf(stderr, "Options for \"islip\" scheduling algorithm:\n");
  ...
  case SCHEDULING_INIT:	
	...
	while( (c = getopt(argc, argv, "n:")) != EOF)
	  switch (c)
	    ...
	    case '?':
	      fprintf(stderr, "-------------------------------\n");
	      fprintf(stderr, "islip: Unrecognized option -%c\n", optopt);
	      islip(SCHEDULING_USAGE);
	      fprintf(stderr, "--------------------------------\n");
	      exit(1);
	    default:
	      break;
  ...
  if(debug_algorithm)
    printf("Algorithm 'islip()' completed for switch %d\n", aSwitch->switchNumber);
}

...
char **argv;
{
	...
	if(debug_algorithm)
		printf("Algorithm 'lpf()' called by switch %d\n", aSwitch->switchNumber);
	
	switch(action) {
		case SCHEDULING_USAGE:
			fprintf(stderr, "No Options for \"lpf\" scheduling algorithm:\n");
			break;
	...
	if(debug_algorithm)
		printf("Algorithm 'lpf()' completed for switch %d\n", aSwitch->switchNumber);
}
	...
	for(input=0; input<aSwitch->numInputs; input++)	{
	  ...
	  for(output=0; output<aSwitch->numOutputs; output++){
		 if(inputBuffer->fifo[output]->number >0){
			...
			/*			graph[input][output]= col_sum[output]+row_sum[input]; */
			graph[input][output]= 10000-(col_sum[output]+row_sum[input]);
			/*

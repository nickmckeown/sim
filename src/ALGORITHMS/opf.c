...
char **argv;
{
	...
	if(debug_algorithm)
		printf("Algorithm 'opf()' called by switch %d\n", aSwitch->switchNumber);
	
	switch(action) {
		case SCHEDULING_USAGE:
			fprintf(stderr, "No Options for \"opf\" scheduling algorithm:\n");
			break;
	...
	if(debug_algorithm)
		printf("Algorithm 'opf()' completed for switch %d\n", aSwitch->switchNumber);
}

...
char **argv;
{
	...
	if(debug_algorithm)
		printf("Algorithm 'mucf()' called by switch %d\n", aSwitch->switchNumber);
	
	switch(action) {
		case SCHEDULING_USAGE:
            fprintf(stderr, "Options for \"mucf\" scheduling algorithm:\n");
            fprintf(stderr, "    -n number_of_iterations. Default: 1\n");
            fprintf(stderr, "NOTE: \"mucf\" requires the use of \"defaultInputAction\" with the -o option.\n");
            break;
		case SCHEDULING_INIT:	
			...
			if( aSwitch->scheduler.schedulingState == NULL )
			{
                ...
                switch (c)
                {
					...
					case '?':
						fprintf(stderr, "-------------------------------\n");
						fprintf(stderr, "mucf: Unrecognized option -%c\n", optopt);
						mucf(SCHEDULING_USAGE);
						fprintf(stderr, "--------------------------------\n");
						exit(1);
                    default:
                        break;
	...
	if(debug_algorithm)
		printf("Algorithm 'mucf()' completed for switch %d\n", aSwitch->switchNumber);
}
...
*/
	...
	long maxUrgencyValue=9999;
	long urgency;
	static int *maxUrgency=(int *) NULL;
	int numEqual=0;
	int selection;

	if( maxUrgency == (int *) NULL)
		maxUrgency = (int *) malloc( (1+aSwitch->numInputs) * sizeof(int));
	...
	/* Pick input with longest waiting time */
	/* If two or more the same, pick first in list */
	...
	for(input=0; input<aSwitch->numInputs; input++)
	{
		...
		if ( scheduleState->inputSched[input]->accept == NONE )
		{
			...
			{
				anElement = inputBuffer->fifo[output]->head;
				aCell = anElement->Object;
				urgency = aCell->commonStats.oqDepartTime - now;
				if(urgency < maxUrgencyValue) 
				{
					numEqual = 0;
					maxUrgencyValue  = urgency;
					maxUrgency[numEqual] = input;
				}
				else if(urgency == maxUrgencyValue)
				{
					numEqual++;
					maxUrgency[numEqual] = input;
				}
...
*/
	...
	if( maxUrgency == (int *) NULL)
		maxUrgency = (int *) malloc( (1+aSwitch->numOutputs) * sizeof(int));
	...
	{
		/* XXXX output = maxUrgency[ (int)lrand48()%(numEqual+1) ]; */
		output = maxUrgency[0];
	}

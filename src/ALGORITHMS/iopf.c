...
static int **findIopfMatch();
static int **iopf_match();
static    int intcompare();


void
iopf(action, aSwitch, argc, argv)
SwitchAction action;
Switch *aSwitch;
int argc;
char **argv;
{
	...
	if(debug_algorithm)
		printf("Algorithm 'iopf()' called by switch %d\n", aSwitch->switchNumber);
	
	switch(action) {
		case SCHEDULING_USAGE:
			fprintf(stderr, "No Options for \"iopf\" scheduling algorithm:\n");
			break;
	...
	if(debug_algorithm)
		printf("Algorithm 'iopf()' completed for switch %d\n", aSwitch->switchNumber);
}
...
Switch *aSwitch;
{
	...
	match = iopf_match(n,newgraph, aSwitch);

	/*	newgraph= iopf_match(n,graph, aSwitch);*/

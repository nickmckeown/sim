...
void
pri_ocf(action, aSwitch, argc, argv)
  SwitchAction action;
Switch *aSwitch;
int argc;
char **argv;
{
  ...
  if(debug_algorithm)
    printf("Algorithm 'pri_ocf()' called by switch %d\n", aSwitch->switchNumber);
	
  switch(action) {
  case SCHEDULING_USAGE:
    fprintf(stderr, "No Options for \"pri_ocf\" scheduling algorithm:\n");
    break;
      ...
      for(output=0; output<numFabricOutputs; output++)
	{
	  for(input=0; input<aSwitch->numInputs; input++)
	      ...
	      if(match[input][output])
		{
		  ...
		  if(input!=NONE) {
           /* In an input is chosen, choose the cell with the highest priority  - Sundar */
  ...
  if(debug_algorithm)
    printf("Algorithm 'pri_ocf()' completed for switch %d\n", aSwitch->switchNumber);
}
      ...
      for(output=0; output<aSwitch->numOutputs; output++)
	{
	  for(index=0,
	      ...
	      index++, fabricOutput++)
	    {
           ...
           /* Always advertise the age of the cell in the highest priority
			queue first */

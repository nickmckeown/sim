...
void
pri_lqf(action, aSwitch, argc, argv)
  SwitchAction action;
Switch *aSwitch;
int argc;
char **argv;
{
  ...
  if(debug_algorithm)
    printf("Algorithm 'pri_lqf()' called by switch %d\n", aSwitch->switchNumber);
	
  switch(action) {
  case SCHEDULING_USAGE:
    fprintf(stderr, "No Options for \"pri_lqf\" scheduling algorithm:\n");
    break;
  ...
  case SCHEDULING_EXEC:
    {
      ...
      int numFabricOutputs = aSwitch->numOutputs*aSwitch->fabric.Xbar_numOutputLines;
      ...
      for(output=0; output<numFabricOutputs; output++)
	{
	  for(input=0; input<aSwitch->numInputs; input++)
	      ...
	      if(match[input][output])
		{
		  ...
		  if(input!= NONE) {
             /* In an input is chosen, choose the cell with the highest
				priority  - Sundar */
			...
			if (selectedpriority == NONE ) 
			  { 
			   ...
			   fprintf(stderr, "INput selected without any cell destined to output \n");
			   exit(1);
			   }
  ...
  if(debug_algorithm)
    printf("Algorithm 'pri_lqf()' completed for switch %d\n", aSwitch->switchNumber);
}
	  ...
	  for(index=0,
	      ...
	      index++, fabricOutput++)
	    {
         ...
         for(priority=0;priority<aSwitch->numPriorities;priority++)
		  ...
		  totalnumber = totalnumber + inputBuffer->fifo[pri]->number;
		  }
	      graph[input][fabricOutput]= totalnumber;
	      weight += totalnumber;
	    }
	}
    }

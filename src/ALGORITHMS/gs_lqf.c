...
extern int **gsaMatch();
static int **findgsaMatch();


void
gs_lqf(action, aSwitch, argc, argv)
  SwitchAction action;
Switch *aSwitch;
int argc;
char **argv;
{
  ...
  if(debug_algorithm)
    printf("Algorithm 'gs_lqf()' called by switch %d\n", aSwitch->switchNumber);
	
  switch(action) {
  case SCHEDULING_USAGE:
    fprintf(stderr, "No Options for \"gs_lqf\" scheduling algorithm:\n");
    break;
  ...
  if(debug_algorithm)
    printf("Algorithm 'gs_lqf()' completed for switch %d\n", aSwitch->switchNumber);
}
      ...
      for(output=0; output<aSwitch->numOutputs; output++)
	{
	  for(index=0,
		fabricOutput=output*aSwitch->fabric.Xbar_numOutputLines;
	      ...
	      index++, fabricOutput++)
	    {
	      graph[input][fabricOutput]=inputBuffer->fifo[output]->number;
	      weight += inputBuffer->fifo[output]->number;
	    }
	}
    }
  /* printf(" %d ", weight); */

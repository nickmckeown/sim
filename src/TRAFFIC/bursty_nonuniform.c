...
/* 
 *  Nonuniform packet bursty model 
 *  Starts idle. 
...
int 
bursty_nonuniform(action, aSwitch, input, argc, argv)
...
char **argv;
{
  ...
  switch(action) 
    {
    case TRAFFIC_USAGE:
      fprintf(stderr, "Options for \"bursty_nonuniform\" traffic model:\n");
      fprintf(stderr, "    -u u0 u1 u2 u3 ... uN-1. [Seperate with spaces.]");
	  fprintf(stderr, "    -p number of prioritylevels of traffic generated. Default: %d\n", priorityLevels);
	  fprintf(stderr, "    -r r0:r1:r2:...rN-1 ratio of unicast utilization in  a priority level \n");
      ...
      fprintf(stderr, "    -R r0:r1:r2:r3... rP-1, multicast utilization ratio per priority level, defaults to unicast utilization per priority level if not specified\n");
	...
	if( debug_traffic)
	  printf("bursty_nonuniform: Traffic init for switch %d input %d\n", aSwitch->switchNumber, input);
	  ...
	  switch(c)
		...
		case 'p':          
		  ...
		  if (priorityLevels > aSwitch->numPriorities) {
			fprintf(stderr,"input %d: number of traffic priorities %d is greater than number of switch priorities %d\n",input,priorityLevels,aSwitch->numPriorities);
			exit(1);
		  }
		  break;
	    case 'd':	
	    ...
	    case '?':
	      fprintf(stderr, "-------------------------------\n");
	      fprintf(stderr, "bursty_nonuniform: Unrecognized option -%c\n", optopt);
	      bursty_nonuniform(TRAFFIC_USAGE);
	      fprintf(stderr, "--------------------------------\n");
	      exit(1);
	    default:
	      break;
	...
	if(rflag &&  !pflag ) {
	  fprintf(stderr,"bursty_nonuniform:\n");
	...
	idleLength = burstLength*(1-traffic->totalUtilization)/traffic->totalUtilization;
...
traffic->totalUtilization;
    ...
    case TRAFFIC_GENERATE:
      {
	if( debug_traffic)
	  printf("        bursty_nonuniform: GEN_TRAFFIC: ");
	...
	if(debug_traffic)
	  printf("bursty_nonuniform: Input %d has cell for %d\n", 
		 input, output);
...
int numOutputs;
{
  ...
  if((! traffic->changeDestPerCell) &&
	 (erand48(traffic->switch_seed) < traffic->probDestinationSwitch )) {
	  if(traffic->cellTypeCurrentBurst == UCAST) {
		/* newpsend = erand48(traffic->out_seed)*traffic->totalUtilization; */
			/*		    for(traffic->lastOutput=0; traffic->lastOutput<numOutputs-1; traffic->lastOutput++) {
									  if(debug_traffic)
									  printf("newpsend %f, utilization: %f\n", 
					   newpsend, traffic->utilization[traffic->lastOutput]); 
			  if( newpsend < traffic->utilization[traffic->lastOutput] )
				break;
				}*/
